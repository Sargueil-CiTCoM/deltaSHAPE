...
def open_map(filename, front, back, sep=None):
    ...
    seq = [str(i.split(sep)[3]) for i in datalines]
    sequence = "".join(seq)
...
def main(argv):
    ...
    required = parse.add_argument_group(
        "Input",
        "input options. files are required in order to run deltaSHAPE analysis.",
    )
    required.add_argument("mapFile1", type=str, help="SHAPE-MaP .map file")
    required.add_argument(
        "mapFile2",
        type=str,
        help="SHAPE-MaP .map file, values in this file will be subtracted from those in mapFile1",
    )
    required.add_argument(
        "--sep",
        type=str,
        default=None,
        help="separator used between .map columns",
    )

    data_opt = parse.add_argument_group(
        "Data manipulation",
        "Options to specify how SHAPE-MaP data are manipulated and analyzed.",
    )
    data_opt.add_argument(
        "--mask5",
        type=int,
        default=0,
        help="Specify the number of nucleotides at the 5' end to ignore. Default: 0",
    )
    data_opt.add_argument(
        "--mask3",
        type=int,
        default=0,
        help="Specify the number of nucleotides at the 3' end to ignore. Default: 0",
    )
    data_opt.add_argument(
        "-p",
        "--pad",
        type=int,
        default=1,
        help="Indicate the smoothing window size. Window = 2*pad+1. To turn off smoothing, set PAD = 0. Default: 1",
    )
    data_opt.add_argument(
        "-z",
        "--Zcoeff",
        type=float,
        default=1.96,
        help="Ajust the Z-factor stringency by changing the equation coefficient. See the README for details. Default: 1.96",
    )
    data_opt.add_argument(
        "-t",
        "--Zthresh",
        type=float,
        default=0,
        help="Adjust the Z-factor stringency by changing the cutoff threshold. See the README for details. Default: 0",
    )
    data_opt.add_argument(
        "-s",
        "--SSthresh",
        type=float,
        default=1,
        help="Set the cutoff threshold of standard score filtering. Default: 1.0",
    )
    data_opt.add_argument(
        "-f",
        "--FindSite",
        type=str,
        default="2,3",
        help="Comma-separated pair of numbers indicating the window pad size and number of required hits when finding binding sites. Default settings look for 3+ nucleotides within a 5-nucleotide window. See the README for details. Default: 2,3",
    )

    out_opt = parse.add_argument_group(
        "Output", "Options specifying plotting and output file details."
    )
    out_opt.add_argument(
        ...
        help="Name and location of output file to be written. Default: ./differences.txt",
    )
    out_opt.add_argument(
        "--magrank",
        action="store_true",
        help="Sort output file by decreasing deltaSHAPE magnitude. Default: OFF",
    )
    out_opt.add_argument(
        "--all",
        action="store_true",
        help="Output data for all nucleotides. Insignificant changes are listed as zero. Default: OFF",
    )
    out_opt.add_argument(
        "--pdf",
        action="store_true",
    ...
    out_opt.add_argument(
        "--noplot",
        action="store_true",
        help="Skip plotting completely. Default: OFF",
    )
    out_opt.add_argument(
        "--dots",
        action="store_true",
        help="Plot markers indicating nucleotides that pass Z-factor and standard score filtering. This can get unweildy for large RNAs (>1000). Standard score (open) dots are plotted above Z-factor (filled) dots. Default: OFF",
    )
    out_opt.add_argument(
        "--Zdots",
        action="store_true",
        help="Plot markers indicating only nucleotides that pass Z-factor filtering. Default: OFF",
    )
    out_opt.add_argument(
        "--SSdots",
        action="store_true",
        help="Plot markers indicating only nucleotides that pass standard score filtering. Default: OFF",
    )
    out_opt.add_argument(
        "--colorfill",
        action="store_true",
        help='Highlight deltaSHAPE sites with coloration beneath the plot line for "prettier" figures. Default: OFF',
    )
    out_opt.add_argument(
        "--ymin",
        type=float,
        default=-999,
        help="Set plot y-axis minimum. Default: Determined automatically",
    )
    out_opt.add_argument(
        "--ymax",
        type=float,
        default=-999,
        help="Set plot y-axis maximum. Default: Determined automatically",
    )
    out_opt.add_argument(
        "--xmin",
        type=float,
        default=-999,
        help="Set plot x-axis minimum. Default: Determined automatically",
    )
    out_opt.add_argument(
        "--xmax",
        type=float,
    ...
    if z_thresh > 1:
        sys.exit(
            "ERROR: Z-factor can never exceed 1. Change -t/--Zthresh value accordingly."
        )
    ...
    site_pad = int(
        args.FindSite.split(",")[0]
    )  # leads to window size used in searching for significant hits
    site_min = int(
        args.FindSite.split(",")[1]
    )  # set minimum number of hits within window
    if site_pad * 2 + 1 < site_min:
        sys.exit(
            "ERROR: Binding site window size and hit minimum are incompatible.\nDouble-check the -f --FindSite flag or consult the README file."
        )
    ...
    """STEP ONE"""
    # open .map files
    data1, err1, seq1 = open_map(args.mapFile1, front, back, sep=sep)
    data2, err2, seq2 = open_map(args.mapFile2, front, back, sep=sep)
    ...
    for region in pos_consec:
        ...
        pos_span.append([region[0] + 0.5, region[-1] + 1.5])
    ...
    for region in neg_consec:
        ...
        neg_span.append([region[0] + 0.5, region[-1] + 1.5])
    ...
    if not args.noplot:
        ...
        plt.axvspan(
            len(diff) - back + 0.5, len(diff) + 0.5, color="grey", alpha=0.25
        )
        ...
        if color == "fill":
            for i in range(len(pos_shade_bits)):
                plt.fill_between(
                    pos_x_bits[i], pos_shade_bits[i], color="#3EB452"
                )
            for i in range(len(neg_shade_bits)):
                plt.fill_between(
                    neg_x_bits[i], neg_shade_bits[i], color="#7F3B95"
                )
        elif color == "bar":
            for i in pos_span:
        ...
        if dots:
            for i in range(len(z_scores)):
                if (args.dots or args.SSdots) and abs(
                    z_scores[i]
                ) >= ss_thresh:
                    plt.scatter(
                        i + 1,
    ...
    if args.all:
        ...
        for i in filter(
            lambda x: x + 1 not in [j[0] for j in data_out], range(len(seq1))
        ):
            data_out.append(
                [
                    i + 1,
